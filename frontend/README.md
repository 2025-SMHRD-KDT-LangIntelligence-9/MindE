...
│   ├── auth.js                # 로그인 / 회원가입 / 비밀번호 재설정 / 아이디 찾기 / 내 정보 / 탈퇴
...
│   ├── citizen/
│   │   ├── Landing.jsx         # 랜딩 페이지
│   │   ├── Login.jsx           # 로그인 (JWT) + 아이디·비밀번호 찾기 모달
│   │   ├── Register.jsx        # 회원가입 (시민 / 담당자)
│   │   ├── Home.jsx            # 시민 대시보드
│   │   ├── Chatbot.jsx         # AI 민원 상담 챗봇 (STT/TTS/이미지/파일, 유사 사례 카드, 중요정보 강조)
...
- **AI 챗봇 민원 상담** — 텍스트/이미지/파일 입력 → AI 분석 → 자동 카테고리·부서 분류
  - 답변의 **중요 정보 자동 강조** (전화번호·법령/조항·부서명을 굵게 표시)
  - **빠른 답변 버튼** (새 상담 시 이용 가이드만 표시, 메시지 전송 후 필요 서류·절차·유사 사례·상담 종료 전체 표시)
  - **상담 종료 시 확인 모달** 후 새 채팅으로 이동
  - **오른쪽 사이드바** — 상담 중 민원 유형·담당 부서·AI 위험도·행정 절차·유사 사례 실시간 표시
- **유사 사례 보기** — 클릭 시 유사 민원 사례(정부24/국민신문고)를 **채팅 카드**로 표시 (유사도 % 포함)
  - 백엔드 `grouped_cases` 지원 시 **카테고리 그룹별로 구분**하여 표시, 없으면 flat 목록 fallback
- **민원 접수 (다중 초안)** — AI가 생성한 초안을 N건 카드로 표시, 각 카드 개별 접수 가능
  - 접수 중인 카드에 **로딩 오버레이** 표시, 다른 카드 버튼은 접수 완료 전까지 비활성화
...
- **계정 찾기** — 로그인 페이지에서 아이디(이메일)·비밀번호 찾기 (이름·전화번호 본인확인)
...
### 관리자
- **대시보드** — 요일별/월별 부서 접수현황(토글 선택), 카테고리별 도넛 차트, 긴급 민원 목록
- **통계 분석** — 상태별·카테고리별·부서별 분포, 긴급도 분포, 응답 지표
- **사용자 관리** — 전체 회원 조회, 담당자 가입 승인/거절, 부서 배정
- **시스템 설정** — 민원 카테고리 / 조직 및 부서 CRUD, **부서 목록 정렬** (부서명·담당자 수·접수·처리 완료·상태 기준 오름차순/내림차순)
...
- 로그인 성공 시 JWT 토큰을 **`sessionStorage`에 저장** (탭 단위로 격리 — 창을 여러 개 열어도 서로 다른 사용자 세션이 섞이지 않음)
...
- 탭을 닫으면 세션이 만료됨 (`sessionStorage` 특성)
...
- **커스텀 스크롤바** — 기본 화살표 버튼 제거 + 얇고 둥근 스타일을 `src/index.css`에서 전역 적용.
