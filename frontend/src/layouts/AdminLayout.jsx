...
function AdminLayout({ pageTitle, activeMenu, children }) {
  ...
  const menuItems = [
    { key: 'dashboard',  label: '대시보드',     short: '대시보드', icon: 'dashboard',     path: '/admin' },
    { key: 'monitoring', label: '민원 모니터링', short: '모니터링', icon: 'monitor_heart', path: '/admin/monitoring' },
    { key: 'stats',      label: '통계 분석',    short: '통계',    icon: 'leaderboard',   path: '/admin/stats' },
    { key: 'settings',   label: '시스템 설정',  short: '설정',    icon: 'settings',      path: '/admin/settings' },
  ];
  ...
  return (
    <div className="min-h-screen text-on-background xl:min-w-[1280px]">
      ...
      <header className="fixed top-0 left-0 w-full h-12 xl:h-16 bg-slate-100 border-b border-slate-200 z-50 flex items-center px-3 xl:px-6 shadow-sm xl:min-w-[1280px]">
        ...
        <button onClick={() => navigate('/admin')} className="shrink-0 xl:mr-6 flex items-center gap-2.5">
          <img src={logo} alt="마음이 로고" className="h-9 xl:h-16 w-auto" />
          <span className="text-[10px] xl:text-xs font-bold text-slate-500 border border-slate-300 px-1.5 xl:px-2 py-0.5 rounded-md">관리자</span>
        </button>
        ...
        <div className="flex items-center gap-2 shrink-0 ml-auto xl:ml-0 xl:pl-4">
          ...
          <div className="flex items-center gap-2 pl-2 xl:pl-3 border-l border-slate-300">
            <span className="hidden sm:inline text-sm font-bold text-slate-700">시스템 관리자</span>
            <button
