...
[chatbot_service.answer_chatbot]
   ...
   │      "포트홀 신고 + 자동차세 감면" 같은 복합 민원 → 서브 민원 리스트로 분해
   │      단일 민원이면 [원문] 1개짜리 리스트
   │
   │  ─── 각 서브 민원마다 병렬 처리 (asyncio.gather) ───
   │  ─── [tool_query 생성] history 직전 user 발언 2개 + 현재 서브 텍스트 합침 ───
   │      → 후속 질문에서 컨텍스트 유지 (A안)
   │
...
## LLM 호출 위치 (신규 Query Decomposition 반영)

| 단계 | 호출 | history 받음? | 비고 |
|---|---|---|---|
| 0 (게이트) | gpt-4o | ✅ | 잡담/민원 1차 판정 |
| Decomp (분해) | gpt-4o | ✅ (최근 3턴) | 복합 민원 분해 (신규) |
| 1 (extract_keywords) | gpt-4o | ❌ | 서브당 1회. tool_query 받음. |
| 3 (답변 LLM) | gpt-4o | ✅ | 최종 답변 생성 |

- **잡담**: 게이트 1회만 → 1회
- **단일 민원**: 게이트 + 분해 + 키워드 + 답변 = **4회**
- **복합 민원 (N 서브)**: 게이트 + 분해 + 서브당 키워드 N회(병렬) + 답변 = **3 + N회**

멀티모달 (음성/이미지) 사용 시 +1~2회 추가.
