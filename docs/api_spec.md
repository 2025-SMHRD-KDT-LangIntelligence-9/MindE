...
게이트(잡담/민원 판정) → **Query Decomposition** (복합 민원 자동 분리) → 서브 질문별 도구 병렬 → 답변 LLM. 멀티턴 지원.
...
```json
{
  ...
  "metadata": {
    ...
    "similar_depts": [...],
    "sub_queries": [               // ⭐ 신규 — 복합 민원 분해 결과
      {
        "query": "포트홀 신고",
        "classification": {...},
        "urgency": {...}, "cluster": {...}, "keywords": [...],
        "laws": [...], "cases": [...],
        "departments": [{"name": "교통행정과", "phone": "061-286-7450", ...}],
        "similar_depts": [...]
      },
      {
        "query": "자동차세 감면 문의",
        "classification": {...},
        "departments": [{"name": "세정과", "phone": "061-286-3610", ...}],
        ...
      }
    ]
  }
...
**단일 민원 vs 복합 민원**:
- 단일이면 `sub_queries`는 1개짜리 리스트. top-level 필드가 그 결과와 동일.
- 복합이면 `sub_queries`가 N개. top-level 필드는 첫 서브만 (하위 호환).
...
**환경변수 필수**: `OPENAI_API_KEY`, `OPENAI_MODEL` (기본 gpt-4o), `HF_TOKEN` (모델 다운로드용)

**내부 흐름**
1. 게이트 LLM (gpt-4o) — 잡담이면 즉답, 민원이면 [TOOL]
2. Query Decomposition LLM (gpt-4o) — 텍스트를 서브 민원 리스트로 분해
